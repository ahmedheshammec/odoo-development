...
export class EmiproDashboardGraph extends Component {
    ...
    renderChart(currentTarget) {
        ...
        const parentDiv = currentTarget.closest('div');
        parentDiv.querySelector('.ep_graph_details .col-3 > h4 span').innerHTML = this.graph_data
        .currency_symbol + this.graph_data.total_sales;
        if(this.graph_data.total_sales && this.graph_data.order_data && this.graph_data.order_data.order_count){
            if(this.graph_data.order_data.order_count != 0){
                parentDiv.querySelector('.ep_graph_details .col-5 b').innerHTML = this.graph_data.currency_symbol + Math.round(this.graph_data.total_sales / this.graph_data.order_data.order_count);
            } else {
                parentDiv.querySelector('.ep_graph_details .col-5 b').innerHTML = this.graph_data.currency_symbol + this.graph_data.order_data.order_count;
            }
        }
        if(this.graph_data && this.graph_data.graph_sale_percentage && this.graph_data.sort_on != 'all'){
            parentDiv.querySelector('.ep_graph_details .col-4 img').setAttribute('src', this.graph_data.graph_sale_percentage.type == 'positive' ? '/common_connector_library/static/src/img/growth-up.svg' : '/common_connector_library/static/src/img/growth-down.svg')
            parentDiv.querySelector('.ep_graph_details .col-4 h4').setAttribute('style', this.graph_data.graph_sale_percentage.type == 'positive' ? 'color:#5cbc2a' : 'color:#ff5a5a')
            parentDiv.querySelector('.ep_graph_details .col-4 h4 b > span:first-child').innerHTML = this.graph_data.graph_sale_percentage.value;
        }

        parentDiv.querySelector('#instance_product > p:first-child').innerHTML = this.graph_data.product_date && this.graph_data.product_date.product_count ? this.graph_data.product_date.product_count: 0;
        parentDiv.querySelector('#instance_customer > p:first-child').innerHTML = this.graph_data.customer_data && this.graph_data.customer_data.customer_count ? this.graph_data.customer_data.customer_count : 0;
        parentDiv.querySelector('#instance_order > p:first-child').innerHTML = this.graph_data.order_data && this.graph_data.order_data.order_count ? this.graph_data.order_data.order_count : 0;
        parentDiv.querySelector('#instance_order_shipped > p:first-child').innerHTML = this.graph_data.order_shipped && this.graph_data.order_shipped.order_count ? this.graph_data.order_shipped.order_count : 0;
        parentDiv.querySelector('#instance_refund > p:first-child').innerHTML = this.graph_data.refund_data && this.graph_data.refund_data.refund_count ? this.graph_data.refund_data.refund_count : 0;
    }
    getLineChartConfig() {
