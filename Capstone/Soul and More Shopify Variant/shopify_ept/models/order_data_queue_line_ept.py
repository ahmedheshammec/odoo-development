...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    def filter_order_queue_lines_and_post_message(self, queues):
        ...
        for queue in queues:
            ...
            queue.write({'queue_process_count': queue.queue_process_count + 1})
    ...
    def process_import_order_queue_data(self, update_order=False):
        ...
        if queue_id:
            queue_id = queue_id.exists()
        if not queue_id:
            _logger.warning(
                "Skipping order queue line(s) %s because parent queue is missing.",
                self.ids,
            )
            self.write({"state": "failed", "processed_at": datetime.now()})
            return True
        if queue_id:
            ...
            queue_id.write({'is_process_queue': True})
