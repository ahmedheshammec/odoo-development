...
class ShopifyProductProductEpt(models.Model):
    ...
    def search_odoo_product_and_set_sku_barcode(self, template_attribute_value_ids, variation, product_template):
        ...
        if list_price and odoo_product:
            odoo_product.write({"lst_price": list_price})
    ...
    def import_shopify_stock_scheduler(self):
        """
        This method is used to import product stock from shopify store to Odoo.
        @author: Angel Patel @Emipro Technologies Pvt. Ltd.
        Migration done by Meera Sidapara on 30/09/2021
        ...
        instance_id = self.env['shopify.instance.ept'].search([('shopify_company_id', '=', company_id)],limit=1)
        # instance=self.env['shopify.instance.ept'].browse(instance_id.id)
        self.import_shopify_stock(instance_id,validate_inventory=False)
