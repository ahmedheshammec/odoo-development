...
class SaleOrder(models.Model):
    ...
    def create_shopify_order_lines(self, lines, order_response, instance):
        """
        This method creates sale order line for Shopify order.
        @author: Maulik Barad on Date 11-Sep-2020.
        """
    ...
    def import_shopify_orders(self, order_data_lines, instance):
        ...
        for order_data_line in order_data_lines:
            ...
            if not order_data or not isinstance(order_data, (str, bytes, bytearray)):
                message = "Order queue line %s has empty or invalid order_data; skipping." % (
                    order_data_line.id,
                )
                _logger.info(message)
                common_log_line_obj.create_common_log_line_ept(
                    ...
                    message=message,
                    model_name='sale.order',
                    order_ref=order_data_line.name,
    ...
    def shopify_create_order(self, instance, partner, shipping_address, invoice_address,
        ...
        total_discount = float(order_response.get("total_discounts", 0.0))
        if float(total_discount) > 0.0:
            ...
            order.env['sale.order.line'].create(discount_line_vals)
    ...
    def shopify_create_sale_order_line(self, line, product, quantity, product_name, price,
        ...
        sale_order_line_obj = self.env["sale.order.line"]
        instance = self.shopify_instance_id
        line_vals = self.prepare_vals_for_sale_order_line(product,order_response, product_name, price, quantity)
    ...
    def prepare_vals_for_sale_order_line(self, product,order_response, product_name, price, quantity):
        ...
        # if product_code == 'shopifyshippingproduct':
        #     discount_percent = 0.0
        # else:
        #     if float(total_discount) > 0:
        #         discount_percent = (float(total_discount) / float(price)) * 100
        #     else:
        #         discount_percent = 0.0
