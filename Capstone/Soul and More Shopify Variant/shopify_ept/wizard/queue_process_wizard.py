...
class ShopifyQueueProcessEpt(models.TransientModel):
    ...
    def process_order_queue_manually(self):
        ...
        queue_line_active_ids = []
        order_queue_ids = order_queue_data_obj.browse(self._context.get('active_ids')).exists().filtered(
            lambda x: x.state != 'done')
        if model == 'shopify.order.data.queue.line.ept':
            queue_line_active_ids.extend(self._context.get('active_ids'))
            orphan_lines = order_queue_ids.filtered(
                lambda line: not line.shopify_order_data_queue_id.exists()
            )
            if orphan_lines:
                orphan_lines.write({'state': 'failed', 'processed_at': datetime.now()})
            order_queue_ids = order_queue_ids.mapped('shopify_order_data_queue_id').exists().filtered(
                lambda queue: queue.state != 'done')
        for order_queue_id in order_queue_ids:
            order_queue_line_ids = order_queue_id.order_data_queue_line_ids.exists().filtered(
                lambda x: x.state in ['draft', 'failed'])
            if queue_line_active_ids:
